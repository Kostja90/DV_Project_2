...
```{r = headWordclod, cache=TRUE, warning=FALSE}
# show only head of frequency dataFrame
head(d,8)
```

Together with this table and the wordcloud one could have a better understanding of the distribution of the artistnames in the given dataset. Now it is interesting to get some more facts about teh most common name John [°research] After a small search on the internet (https://en.wikipedia.org/wiki/List_of_most_popular_given_names#Male_names_2) one can see, that John was one of the most common names in the 1990's. 
