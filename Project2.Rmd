...
As mentioned before the selfseted goal of this task, was to find similarities of approved songs and to compare those to a recomendation made by spotify. To achive this, one should be able to read in the provided data in HDF5 Format. Therefor it is required to load the package 'rhdf5' [^package2] that helps accessing the datafiles and get the information out to analize it. 
The chossen data is a preferation of a person of our group. The songs he choosed are ordered as followed:  <br />

- Beynonce Single Ladies TrackID: TRAPZTV128F92CAA4E  <br />

- Justin Timberlake What Goes Arround Comes Arround TrackID: TRANNZZ128F92C22F7  <br />
...
Taking the TrackID that functions also as the filename, one should first find out the right paths to the prefered files to access them. After creating the right path one should be able to access the data throug some methods. As described below there are two methods to read in data. The first one is to access the groups with method h5ls() and the second one to access the data in those groups with method h5read(). One can see a slice of an output for h5ls() underneath.
...
This slice shows that there are different groups and even different subgroups one can access through h5read().
An example of the final data of beyonces song of group analysis and metadata can be seen underneath.
...
t(Meta_song['beyonce',])
```
Both tables will be used and combined on different variables. What also draws atention is the sparse vektors in the table above. Obviously one should get rid of those and just use the other variables for comparison.

First, one has to decide which variables one should take and what the meaning of those is. Second, it is important to decide which of those variables is comparable and where the similarities between the songs are. Therefor one needs to make some further detailed analysis on the given data. Finally one should be able to predict any given song if it fits the calculated results and the predicted song would be recomended to that person. 

## Data discription 
By looking at the data without any knowledge in beforehand, one should understand each an every variable. Since this has been done allready we will concentrate on the choosen variables and describe those. <br \>
Variables: <br \>
...
- Loudness: This indicates the overall loudness of a track in decibels (dB) and is avaraged across a track in this dataset. The Loudness is correlated with the physical strength   In the given dataset the Loudnes is avareged across a track [^înfo2]. <br \>

- Mode: In the theory of Western music, a mode is a type of musical scale coupled with a set of characteristic melodic behaviors. [^wiki4]. To simplify this statement a mode could be seen a s the vocabulary of a melody. It specifies which notes can be used and indicates which have special importance [^înfo3]. Mostly Major notes sound happy and Minor notes does not [^spotify] <br \>
...
With all the provided information in the section above, one can derive a more specific representation of all the variables as well as a relationship between them. As can be seen below and also written in various articles, specialy on spotify, Emotion is a very important characteristic of music. Mostly people listen to music when they are sad or happy or try to project their feelings on to it. Therefor Emotion does have specific role when talking about music [^spotify]. In addition to that there is also the Genre of a song and even an artist, that is depending on other variables. It is also a very important characteristic that can predict a listeners favour. Mostly people tend to listen to one or two particular genres. For sure there are more overall variables like Emotion and Genre, but for now we limit ourself to only those two.  

![Self created picture for the relationship between the variables](./picturesDocu/Variable.jpg)

As provided by the image above, that was created by ourself the variables tend to describe the overall variables. Our research on the variables, led to this picture. Here one can see properly that the relationship for Emotion does of course consist of all the variables, since the whole package of information does lead towards a proper and confident answer, but also it is highly dependent on the Key and the mode. As mentiond in the section Data discription especially the mode, mostly tends to describe either if the song is a happy one or not. Here one can see that Key as well is a reliable source for measuring the Emotion, since it does correlate with the mode. Also the pitch is corresponding to one of the 12 keys and is therefor a reliable source for Emotions too.   
